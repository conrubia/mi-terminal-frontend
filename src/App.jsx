...
function App() {
  ...
  const ejecutarBusquedaInteligente = async (e) => {
    ...
    setIsLoading(true);
    try {
        const res = await fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent(`https://query2.finance.yahoo.com/v1/finance/search?q=${searchInput}`)}`);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch de Datos de Yahoo con Proxy AllOrigins para evitar CORS
        const urlYahoo = `https://query1.finance.yahoo.com/v8/finance/chart/${ticker}?range=${range}&interval=1d`;
        const res = await fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent(urlYahoo)}`);
        ...
        const volumeData = candleData.map((d, i) => ({
          time: d.time,
          value: result.indicators.quote[0].volume[i],
        ...
        // 3. CAMBIA ESTO: Pon tu URL de Render (mantén el /api/analisis/${ticker} al final)
        const resNews = await fetch(`https://mi-terminal-backend.onrender.com/api/analisis/${ticker}`); // <--- CAMBIO 3
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '20px', maxWidth: '1400px', margin: '0 auto', height: 'calc(100vh - 40px)' }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ position: 'relative', flexGrow: 1 }}>
            ...
            {isLoading && <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', color: '#26a69a' }}>Cargando terminal...</div>}
            {selectedNews && (
               <div style={{ position: 'absolute', top: '20px', right: '20px', width: '300px', backgroundColor: 'rgba(30, 34, 45, 0.95)', border: `1px solid ${selectedNews.color}`, borderRadius: '8px', padding: '15px', zIndex: 100}}>
                  ...
                  <a href={selectedNews.url} target="_blank" rel="noreferrer" style={{ color: '#2962FF', display: 'block', marginTop: '10px', textDecoration: 'none', fontSize: '12px' }}>Leer más →</a>
                </div>
            )}
          </div>
        </div>
        <div style={{ backgroundColor: '#131722', borderRadius: '8px', border: '1px solid #2B2B43', padding: '15px', display: 'flex', flexDirection: 'column' }}>
           <EconomicCalendar />
           <FearAndGreed />
        </div>
      </div>
